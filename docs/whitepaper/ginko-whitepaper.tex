...
\begin{itemize}
 ...
 \item \textbf{ginko}---lower-level helper CLI for project initialization, diagnostics, and configuration generation.
\end{itemize}
...
\begin{itemize}
 ...
 \item \textbf{CLI.} \texttt{ginko} is the recommended end-user CLI. \texttt{memctl} remains available for scriptable lower-level access to primary operations and non-MCP automation.
\end{itemize}
...
\textit{ginko}~\cite{llmmemoryrepo} & SQLite + FTS5 + events & Local-first & Canonical, append-only events, supersession \\
\bottomrule
\end{tabularx}
...
\begin{itemize}
 ...
 \item \textbf{Contradiction detection is still maturing.} Governance primitives---conflict relations, supersession recommendations, confidence reinforcement, topic keys, and review queues---are central to the roadmap and are being introduced incrementally.
 ...
 \item \textbf{Soft-delete semantics are still maturing.} The roadmap calls for explicit soft-delete/tombstone behavior and lifecycle-aware retrieval. Backup of the SQLite file remains the recommended safety practice until these workflows are stable.
\end{itemize}
...
\begin{itemize}
 \item \textbf{v0.1 --- Local core.} Canonical store, FTS5 retrieval, MCP server, GUI, and the first working local memory model.
 \item \textbf{v0.2 --- Local safety and installability.} API tokens, auth-aware MCP propagation, install scripts, structured logging, smoke tests, and safer local setup.
 \item \textbf{v0.3 --- Agent integration quality.} Tested integrations for Claude Code, Codex-style CLIs, OpenClaw, and generic MCP clients. Progressive-disclosure retrieval (\texttt{memory\_get}, \texttt{memory\_timeline}, and session tools). One-command setup with dry-run.
 \item \textbf{v0.4 --- Governance and memory quality.} Contradiction detection, supersession recommendation, conflict/reinforcement relations, stable \texttt{topic\_key}, status/lifecycle fields, soft delete, audit GUI, sensitive-data detection, and approval queues.
 \item \textbf{Cross-cutting --- Persistent inter-agent coordination.} A separate \texttt{agent\_signals} coordination layer for notices, leases, handoffs, conflicts, review requests, and blockers across heterogeneous agents sharing one local database.
 \item \textbf{v0.5 --- RAG bridge.} Document ingestion, chunk evidence, traceable ingestion runs, candidate extraction from evidence, and citation-aware context.
 \item \textbf{v0.6 --- Retrieval quality.} Hybrid ranking (BM25 + optional embeddings + recency decay parameterized by type), MMR-style diversification, subject-hierarchy aware retrieval, confidence-aware scoring, and retrieval evaluation fixtures.
\end{itemize}
...
\begin{itemize}
 ...
 \item Version family: experimental v0.x
 \item License: MIT
 ...
 \item Distribution: \texttt{ginko} CLI plus lower-level \texttt{ginko}, \texttt{memctl}, \texttt{memmcp}, and \texttt{memserver} binaries
\end{itemize}
