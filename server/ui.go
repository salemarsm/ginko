...
async function loadSettings(){const c=await api('/api/config');document.getElementById('rows').innerHTML='<tr><td colspan="8"><h3>Configuração</h3><table style="max-width:600px"><tr><th>Campo</th><th>Valor</th></tr><tr><td>database</td><td><code>'+esc(c.database.path)+'</code></td></tr><tr><td>server.addr</td><td><code>'+esc(c.server.addr)+'</code></td></tr><tr><td>llm</td><td><code>'+esc(c.llm.provider)+'/'+(c.llm.model||'-')+'</code></td></tr><tr><td>embedding</td><td><code>'+esc(c.embedding.provider)+'/'+(c.embedding.model||'-')+' (index: '+esc(c.embedding.index)+')</code></td></tr><tr><td>auth</td><td>'+(c.server.addr&&!c.server.addr.includes('127.')&&!c.server.addr.includes('localhost')?'<span class="warn">non-loopback — token required</span>':'<span class="muted">loopback</span>')+'</td></tr></table><p class="muted" style="margin-top:12px">Read-only. Edit ~/.ginko/config.json and restart ginko serve to apply changes.</p></td></tr>'}
